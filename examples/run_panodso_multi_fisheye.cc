...
#include "Pano_DatasetReader.h"

using namespace std;
using namespace ldso;

/*********************************************************************************
 * This program demonstrates how to run PANO-DSO in ladybug dataset
...
std::string source = "/home/jiangfan/data/Ladybug/jiejing/02_caihonghuayuanxilu";
std::string output_file = "./results.txt";
std::string calib = "./examples/ladybug_jiejing/Kitti00-02.txt";
...
void parseArgument(char *arg) {
    ...
    if (1 == sscanf(arg, "camNum=%d", &option)) {
        camNum = option;
        LOG(INFO) << "Number of camera is " << camNum << endl;   // fisheye 数量 ，默认ladybug设为5
        return;
    }
...
int main(int argc, char **argv) 
{
    ...
    // we has no photometric calibration of ladybug dataset ,so do calibration-online by applying affine parameters
    ...
    shared_ptr<ImageFolderReader> reader(
            new ImageFolderReader(ImageFolderReader::Fisheye, camNum, source, calib, "", ""));    // no gamma and vignette

    reader->setwh(0, "", "");   // 设置w，h.
    reader->setGlobalCalibration();  //  

    UMF = reader->getCalibration();    // 返回 UndistortMultiFisheye 全景几何类
    ...
    shared_ptr<FullSystem> fullSystem(new FullSystem(voc, UMF));
    ...
    if (!disableAllDisplay) {
        viewer = shared_ptr<PangolinDSOViewer>(new PangolinDSOViewer(UMF->getOriginalSize()(0), UMF->getOriginalSize()(1), UMF->camNums,false));
    ...
    std::thread runthread([&]() {
        ...
        std::vector<std::vector<ImageAndExposure *>> preloadedImages;   //numimage numcameras   
        if (preload) {
        ...
        for (int ii = 0; ii < (int) idsToPlay.size(); ii++) {
            ...
            std::vector<ImageAndExposure *> vimg;
            if (preload)
                vimg = preloadedImages[ii];
            else
                vimg = reader->getImage(i);
            ...
            if (!skipFrame) fullSystem->addActiveFrame(vimg, i);
            vimg.clear();

            if (fullSystem->initFailed || setting_fullResetRequested) {
                if (ii < 250 || setting_fullResetRequested) {
                    LOG(INFO) << "RESETTING!" << endl <<
                         " fullSystem->initFailed:"<< fullSystem->initFailed <<
                         " setting_fullResetRequested :" << setting_fullResetRequested << endl ;
                    fullSystem = shared_ptr<FullSystem>(new FullSystem(voc, UMF));
                    fullSystem->setGammaFunction(reader->getPhotometricGamma());
                    fullSystem->linearizeOperation = (playbackSpeed == 0);
                    if (viewer) {
                        viewer->reset();
