...
namespace ldso {
    ...
    // for multi-frame
    Frame::Frame(double timestamp, UndistortMultiFisheye* u)
    {
        id = nextId++;
        this->timeStamp = timestamp;
        this->UMF = u;
    ...
    // for multi-fisheye
    void Frame::CreateFH(shared_ptr<Frame> frame, UndistortMultiFisheye* u)
    {
        frameHessian = shared_ptr<internal::FrameHessian>(new internal::FrameHessian(frame, u));
    }
