...
namespace ldso {
    ...
    int FeatureDetector::DetectCornersforMF(int nFeatures, shared_ptr<Frame> &frame, int n, UndistortMultiFisheye* UMF) 
    {
        // grid it
        int gridsize = int(sqrtf(UMF->wPR[0] * UMF->hPR[0] / nFeatures) + 0.5);
        int gridX = UMF->wPR[0] / gridsize + 1, gridY = UMF->hPR[0] / gridsize + 1;
        float nfeatInGrid = float(nFeatures) / (UMF->wPR[0] * UMF->hPR[0]) * (gridsize * gridsize);

        float maxScore = 0, scoreTH = 0;
        int skip = (HALF_PATCH_SIZE * 2 / gridsize) + 1;

        for (int gx = skip; gx < gridX - skip; gx++) 
        {  // 最边上的不要
            for (int gy = skip; gy < gridY - skip; gy++) 
            {

                float maxGrad = 0, gradTH = 0;
                float *gradData = &frame->frameHessian->vabsSquaredGrad[0][n][gy * gridsize * UMF->wPR[0] + gx * gridsize];

                for (int x = 0; x < gridsize; x++) 
                {
                    for (int y = 0; y < gridsize; y++) 
                    {
                        if (gradData[y * UMF->wPR[0] + x] > maxGrad)
                            maxGrad = gradData[y * UMF->wPR[0] + x];
                    }
                }
                ...
                for (int x = 0; x < gridsize; x++) 
                {
                    ...
                    {
                        int idx = y * gridsize + x;
                        if (gradData[y * UMF->wPR[0] + x] > gradTH) 
                        {
                            // this is an candidate
                            int realX = gx * gridsize + x, realY = gy * gridsize + y;
                            if(frame->frameHessian->vmask[0][n][realX + realY *UMF->wPR[0]] != 0)
                            {
                                float s = ShiTomasiScore(frame, realX, realY);
                                candidate.push_back(pair<int, float>(idx, s));
                                if (s > maxScore) {
                ...
                for (auto &p: candidate) {
                    ...
                    shared_ptr<Feature> feat(new Feature(x, y, X, Y, Z, frame, n));
                    //shared_ptr<Feature> feat(new Feature(realX, realY, frame));
                    feat->score = p.second;
                    frame->features.push_back(feat);
                    picked++;
