...
namespace ldso {
    ...
    void KeyFrameDisplay::setFromKF(shared_ptr<FrameHessian> fh, shared_ptr<CalibHessian> HCalib) {
        ...
        for (auto feat: fr->features) {
            if (feat->point && feat->point->mpPH) {
                ...
                pc[numSparsePoints].color[0] = p->color[0];
    ...
    // for multi-fisheye
    void KeyFrameDisplay::setFromKFforMF(shared_ptr<FrameHessian> fh, UndistortMultiFisheye* UMF) 
    {

        if (fh->frame) {
            setFromFforMF(fh->frame, UMF);
        } else
            camToWorld = Sim3(fh->PRE_camToWorld.matrix());
        ...
        for (auto feat: fr->features) 
        {
            if (feat->point && feat->point->mpPH) 
            {
                ...
                pc[numSparsePoints].u = p->u;
                pc[numSparsePoints].v = p->v;
                pc[numSparsePoints].xs = p->xs;
                pc[numSparsePoints].ys = p->ys;
                pc[numSparsePoints].zs = p->zs;
                pc[numSparsePoints].idpeth = p->idepth_scaled;
                pc[numSparsePoints].relObsBaseline = p->maxRelBaseline;
                pc[numSparsePoints].idepth_hessian = p->idepth_hessian;
                pc[numSparsePoints].numGoodRes = 0;
    ...
    // for multi-fisheye
    void KeyFrameDisplay::setFromFforMF(shared_ptr<Frame> fs, UndistortMultiFisheye* UMF) 
    {

        id = fs->id;
        double x,y;
        UMF->GetRectifiedImgCenter(0, &x, &y);
        cx = x; cy = y;
        fx = fy = UMF->GetRectifiedImgFocalLength(0, 0);
        width = UMF->wPR[0];
        height = UMF->hPR[0];
        fxi = 1 / fx;
        fyi = 1 / fy;
    ...
    void KeyFrameDisplay::drawCamforMF(float lineWidth, float *color, float sizeFactor, bool drawOrig)
    {
        if (width == 0)
            return;
        ...
        glBegin(GL_LINES);
        glVertex3f(0, 0, 0);
        glVertex3f(sz, sz * (cx) / fx, sz * (cy) / fy);
        glVertex3f(0, 0, 0);
        glVertex3f(sz, sz * (cx) / fx, - sz * (height - 1 - cy) / fy);
        glVertex3f(0, 0, 0);
        glVertex3f(sz, - sz * (width - 1 - cx) / fx, - sz * (height - 1 - cy) / fy);
        glVertex3f(0, 0, 0);
        glVertex3f(sz, - sz * (width - 1 - cx) / fx, sz * (cy) / fy);
    ...
    // for multi-fisheye
    bool KeyFrameDisplay::refreshPCforMF(bool canRefresh, float scaledTH, float absTH, int mode, float minBS, int sparsity, bool forceRefresh)
    {
        if (forceRefresh) {
            needRefresh = true;
        } else if (canRefresh) {
        ...
        // string path = "/home/jiangfan/桌面/pan_dso_calib/pano_inital/pcviewer.txt";
        // ofstream outp;
        ...
        for (int i = 0; i < numSparsePoints; i++) {
            ...
            if (originalInputSparse[i].relObsBaseline < (my_minRelBS * 0.012))
                continue;
            ...
            //for (int pnt = 0; pnt < patternNum; pnt++) {
                ...
                tmpVertexBuffer[vertexBufferNumPoints][0] = originalInputSparse[i].xs/20 * depth;
                tmpVertexBuffer[vertexBufferNumPoints][1] = originalInputSparse[i].ys/20 * depth;
                tmpVertexBuffer[vertexBufferNumPoints][2] = originalInputSparse[i].zs/20 * depth;
                // outp << i << " " << originalInputSparse[i].xs << " " << originalInputSparse[i].ys << " " << originalInputSparse[i].zs << " " << originalInputSparse[i].idpeth << endl;
                //tmpVertexBuffer[vertexBufferNumPoints][2] = depth * (1 + 2 * fxi * (rand() / (float) RAND_MAX - 0.5f));
                ...
                if (my_displayMode == 0) {
                    ...
                    tmpColorBuffer[vertexBufferNumPoints][1] = originalInputSparse[i].color[0];
                    tmpColorBuffer[vertexBufferNumPoints][2] = originalInputSparse[i].color[0];
                }
                vertexBufferNumPoints++;
                assert(vertexBufferNumPoints <= numSparsePoints);

                // vpc.push_back(Vec3f(tmpVertexBuffer[vertexBufferNumPoints][0], tmpVertexBuffer[vertexBufferNumPoints][1], tmpVertexBuffer[vertexBufferNumPoints][2]));
                // vpccolor.push_back(Vec3b(originalInputSparse[i].color[0], originalInputSparse[i].color[0], originalInputSparse[i].color[0]));
            //}
        }
        // outp.close();
    ...
    void KeyFrameDisplay::drawPC(float pointSize) {
        ...
        // string path = "/home/jiangfan/桌面/pan_dso_calib/pano_tracking/pcv3diewer.txt";
        // ofstream outp;
        // outp.open(path.c_str());
        // glBegin(GL_POINTS);
        // for(int p=0;p<vpc.size();p++)
        // {
        //     glColor3f(0.0, 0.0, 0.0);
        //     glVertex3f(vpc[p](0), vpc[p](1), vpc[p](2));
        //     outp << p << " " << vpc[p](0) << " " << vpc[p](1) << " " << vpc[p](2) << endl;
        // }
        // glEnd();
        // outp.close();
    ...
    PangolinDSOViewer::PangolinDSOViewer(int w, int h, int n, bool startRunThread) 
    {
        this->imageNum = n;
        this->w = w;
        this->h = h;
        ...
        unique_lock<mutex> lk(openImagesMutex);
        internalVideoImg = new MinimalImageB3(w*imageNum, h);
        internalKFImg = new MinimalImageB3(w*imageNum, h);
        videoImgChanged = true;
        kfImgChanged = true;
    ...
    void PangolinDSOViewer::run() {
        ...
        pangolin::OpenGlRenderState Visualization3D_camera(
            ...
            pangolin::ModelViewLookAt(-10, -0, 2, 0, 0, 0, pangolin::AxisZ)
        );
        ...
        pangolin::View &d_video = pangolin::Display("imgVideo")
            .SetAspect(w*imageNum / (float) h);

        pangolin::View& d_kfDepth = pangolin::Display("imgKFDepth")
	        .SetAspect(w*imageNum / (float)h);

        pangolin::GlTexture texVideo(w*imageNum, h, GL_RGB, false, 0, GL_RGB, GL_UNSIGNED_BYTE);
        pangolin::GlTexture texKFDepth(w*imageNum,h, GL_RGB,false,0,GL_RGB,GL_UNSIGNED_BYTE);
            ...
            if (setting_render_display3D) {
                ...
                if (needRefreshAll) {
                    for (auto &fh: keyframes) 
                    {
                        ...
                        fh->refreshPCforMF(true, this->settings_scaledVarTH,
                                      this->settings_absVarTH,
                                      this->settings_pointCloudMode, this->settings_minRelBS,
                                      this->settings_sparsity);
                        fh->drawPC(4);
                    ...
                    for (auto fh : keyframes) 
                    {
                        ...
                        //                                   this->settings_pointCloudMode, this->settings_minRelBS,
                        //                                   this->settings_sparsity));
                        refreshed = +(int) (fh->refreshPCforMF(refreshed < 10, this->settings_scaledVarTH,
                                                          this->settings_absVarTH,
                                                          this->settings_pointCloudMode, this->settings_minRelBS,
                                                          this->settings_sparsity));
                        fh->drawPC(4);
                ...
                if (this->settings_showKFCameras) {
                    ...
                    for (auto &id: activeKFIDs) {
                        ...
                        kfd->drawCamforMF(1.5, blue, 0.1);
                    }
                }

                // current cam
                if (this->settings_showCurrentCamera && currentCam)
                    currentCam->drawCamforMF(2, 0, 0.2);
    ...
    // for multi-fisheye
    void PangolinDSOViewer::publishKeyframesforMF(std::vector<shared_ptr<Frame>> &frames, bool final, UndistortMultiFisheye* UMF)
    {
        if (!setting_render_display3D) return;
        if (disableAllDisplay) return;
        ...
        for (auto fr :frames) 
        {
            ...
            {}
            if (fr->frameHessian)
                keyframesByKFID[fr->kfId]->setFromKFforMF(fr->frameHessian, UMF);
            else
                keyframesByKFID[fr->kfId]->setFromFforMF(fr, UMF);
            activeKFIDs.push_back(fr->kfId);
        }
    }
    ...
    void PangolinDSOViewer::publishCamPoseforMF(shared_ptr<Frame> frame, UndistortMultiFisheye* UMF) 
    {
        ...
        last_track = time_now;
        if (currentCam)
            currentCam->setFromFforMF(frame, UMF);
        allFramePoses.push_back(frame);
        ...
        if (frame->frameHessian) 
        {
            ...
            for(int n = 0; n < imageNum; n++)
            {
                for (int i = 0; i < w ; i++)
                    for(int j = 0; j < h; j++)
                {
                    int idx = n * w + i + imageNum * w * j;
                    int idx2 = i + j* w;
                    internalVideoImg->data[idx][0] =
                    internalVideoImg->data[idx][1] =
                    internalVideoImg->data[idx][2] =
                        frame->frameHessian->vdI[n][idx2][0] * 0.8 > 255.0f ? 255.0 : frame->frameHessian->vdI[n][idx2][0] * 0.8;
                        // frame->frameHessian->vdfisheyeI[n][idx2] * 0.8 > 255.0f ? 255.0 : frame->frameHessian->vdfisheyeI[n][idx2] * 0.8;
    ...
    void PangolinDSOViewer::pushLiveFrameforMF(shared_ptr<FrameHessian> frameHessian)
    {
        if (!setting_render_display3D)
            return;
        if (disableAllDisplay)
        ...
        if (frameHessian) 
        {
            ...
            for(int n = 0; n < imageNum; n++)
            {
                    ...
                    for(int j = 0; j < h; j++)
                {
                    ...
                    internalVideoImg->data[idx][1] =
                    internalVideoImg->data[idx][2] =
                        frameHessian->vdI[n][idx2][0] * 0.8 > 255.0f ? 255.0 : frameHessian->vdI[n][idx2][0] * 0.8;
                        // frameHessian->vdfisheyeI[n][idx2] * 0.8 > 255.0f ? 255.0 : frameHessian->vdfisheyeI[n][idx2] * 0.8;
    ...
    void PangolinDSOViewer::pushDepthImageforMF(vector<MinimalImageB3* > image)
    {
         ...
         for(int n = 0; n < imageNum; n++)
            {
                    ...
                    for(int j = 0; j < h; j++)
                {
                    int idx = n * w + i + imageNum * w * j;
                    int idx2 = i + j* w;
                    internalKFImg->data[idx][0] = image[n]->data[idx2][0];
                    internalKFImg->data[idx][1] = image[n]->data[idx2][1];
                    internalKFImg->data[idx][2] = image[n]->data[idx2][2];
                }
            }
        kfImgChanged=true;
    }
