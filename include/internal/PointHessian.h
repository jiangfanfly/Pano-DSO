...
namespace ldso {
    namespace internal {
        ...
        public:
            ...
            int camnum; // which camera the pnt is belonged to
            ...
            // H and b blocks
            float bdSumF = 0;                   //!< 当前残差 + 边缘化先验残差   Jr_rho * r
            float HdiF = 0;                     //!< 逆深度hessian的逆, 协方差
            float Hdd_accLF = 0;                //!< 边缘化, 逆深度的hessian
            VecCf Hcd_accLF = VecCf::Zero();       //!< 边缘化, 逆深度和内参的hessian
            float bd_accLF = 0;                 //!< 边缘化, J逆深度*残差
            float Hdd_accAF = 0;                //!< 正常 逆深度的hessian
            VecCf Hcd_accAF = VecCf::Zero();        //!< 正常逆深度和内参的hessian
            float bd_accAF = 0;                 //!< 正常 J逆深度*残差
