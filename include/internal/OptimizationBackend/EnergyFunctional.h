...
namespace ldso {
    namespace internal {
        ...
        public:
            ...
            MatXX HM = MatXX::Zero(CPARS, CPARS);   // frame-frame H matrix     优化的Hessian矩阵, 边缘化掉逆深度
            VecX bM = VecX::Zero(CPARS);    // frame-frame b vector     优化的Jr项, 边缘化掉逆深度
            ...
            //====================================
            // for multi-fisheye
            EnergyFunctional(UndistortMultiFisheye *uMF);

            void insertFrameforMF(shared_ptr<FrameHessian> fh);

            void setDeltaFforMF();

            void setAdjointsFforMF();

            void solveSystemFforMF(int iteration, double lambda);

            /**
             * compute frame-frame prior energy
             * @return
             */
            double calcMEnergyFforMF();

            /**
             * compute point energy with multi-threading
             * @return
             */
            double calcLEnergyF_MTforMF();
             ...
             * Marg all points to compute Bundle Adjustment
             */
            void marginalizePointsFforMF();

             /**
             * marginalize a given frame
             * @param fh
             */
            void marginalizeFrameforMF(shared_ptr<FrameHessian> fh);
        ...
        private:
            ...
            shared_ptr<AccumulatedTopHessianSSE> accSSE_top_A;   // Hession矩阵 非Schur comlepment
            shared_ptr<AccumulatedSCHessianSSE> accSSE_bot;      // Hession矩阵 Schur comlepment
            ...
            //====================================
            // for multi-fisheye
            VecX getStitchedDeltaFforMF() const 
            {
                VecX d = VecX(nFrames * 16);
                for (int h = 0; h < nFrames; h++)
                    d.segment<16>(16 * h) = frames[h]->deltaforMF;
                return d;
            }

            // 构建 H b
            void accumulateAF_MTforMF(MatXX &H, VecX &b, bool MT);
            void accumulateLF_MTforMF(MatXX &H, VecX &b, bool MT);
            void accumulateSCF_MTforMF(MatXX &H, VecX &b, bool MT);

            void calcLEnergyPtforMF(int min, int max, Vec10 *stats, int tid);

            // 更新变量
            void resubstituteF_MTforMF(const VecX &x, bool MT);
            void resubstituteFPtforMF(Mat156f *xAd, int min, int max, Vec10 *stats, int tid);



            Mat156f *adHTdeltaFforMF = nullptr;     ////!< host和target之间位姿的增量, 一共帧数×帧数个

            Mat5656 *adHostforMF = nullptr;    // arrays of adjoints, adHost = -Adj(HostToTarget)^T   + 光度仿射变换
            Mat5656 *adTargetforMF = nullptr;   // double

            Mat5656f *adHostFforMF = nullptr;   // float
            Mat5656f *adTargetFforMF = nullptr;
