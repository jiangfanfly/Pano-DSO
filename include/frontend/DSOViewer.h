...
namespace ldso {
    ...
    public:
        ...
        // copies points from KF over to internal buffer,
        // keeping some additional information so we can render it differently.
        void setFromKFforMF(shared_ptr<FrameHessian> fh, UndistortMultiFisheye* UMF);

        // copies points from KF over to internal buffer,
        // keeping some additional information so we can render it differently.
        void setFromFforMF(shared_ptr<Frame> fs, UndistortMultiFisheye* UMF);

        // copies & filters internal data to GL buffer for rendering. if nothing to do: does nothing.
        bool refreshPCforMF(bool canRefresh, float scaledTH, float absTH, int mode, float minBS, int sparsity,
                       bool forceRefresh = false);

        // renders cam & pointcloud.
        void drawCamforMF(float lineWidth = 1, float *color = 0, float sizeFactor = 1, bool drawOrig = true);

        void drawPCforMF(float pointSize);
    ...
    public:
        ...
        // ============================================
        // for multi-fisheye
        PangolinDSOViewer(int w, int h, int imagenum, bool startRunThread = true); 

        void pushDepthImageforMF(vector<MinimalImageB3* > image);

        void pushLiveFrameforMF(shared_ptr<FrameHessian> frameHessian);

        void publishKeyframesforMF(std::vector<shared_ptr<Frame>> &frames, bool final, UndistortMultiFisheye* UMF);

        void publishCamPoseforMF(shared_ptr<Frame> frame, UndistortMultiFisheye* UMF);
    ...
    private:
        ...
        MinimalImageB3* internalKFImg = nullptr;
	    MinimalImageB3* internalResImg = nullptr;
        bool videoImgChanged = true;
        bool kfImgChanged = true;
        bool resImgChanged = true;
