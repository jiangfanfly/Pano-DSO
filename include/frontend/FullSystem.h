...
namespace ldso {
    ...
    public:
        ...
        FullSystem(shared_ptr<ORBVocabulary> voc, UndistortMultiFisheye * UMF);
        ...
        // add multi fisheye image, and create point & residual structs
        void addActiveFrame(vector<ImageAndExposure *> image, int id);

        float optimizeforMF(int mnumOptIts);
    ...
    private:
        ...
        /**
         * remove the outliersFeatureDetector
         */
        void removeOutliers();
         ...
         * @return a 4-vector which has some tracking status information, used to judge if we need a keyframe
         */
        Vec4 trackNewCoarseforMF(shared_ptr<FrameHessian> fh);

        void initializeFromInitializerforMF(shared_ptr<FrameHessian> newFrame);
        /**
         * set precalc values.
         */
        void setPrecalcValuesforMF();

        void deliverTrackedFrameforMF(shared_ptr<FrameHessian> fh, bool needKF);

        void makeKeyFrameforMF(shared_ptr<FrameHessian> fh);

        void makeNonKeyFrameforMF(shared_ptr<FrameHessian> &fh);

        void traceNewCoarseforMF(shared_ptr<FrameHessian> fh);

        void flagFramesForMarginalizationforMF(shared_ptr<FrameHessian> &newFH);

        void activatePointsMTforMF();

        void activatePointsMT_ReductorforMF(std::vector<shared_ptr<PointHessian>> *optimized,
                    std::vector<shared_ptr<ImmaturePoint>> *toOptimize, int min, int max, Vec10 *stats, int tid);

        shared_ptr<PointHessian>
        optimizeImmaturePointforMF(shared_ptr<internal::ImmaturePoint> point, int minObs,
                                        vector<shared_ptr<ImmaturePointTemporaryResidual>> &residuals);

        Vec3 linearizeAllforMF(bool fixLinearization);

        void
        linearizeAll_ReductorforMF(bool fixLinearization, std::vector<shared_ptr<PointFrameResidual>> *toRemove, int min,
                              int max,
                              Vec10 *stats, int tid);

        void backupStateforMF(bool backupLastStep);

        void solveSystemforMF(int iteration, double lambda);

        std::vector<VecX> getNullspacesforMF(std::vector<VecX> &nullspaces_pose, std::vector<VecX> &nullspaces_scale);

        /// energy computing functions, called in optimization
        double calcLEnergyforMF();

        /// energy computing functions, called in optimization
        double calcMEnergyforMF();

        bool doStepFromBackupforMF(float stepfacT, float stepfacR, float stepfacA, float stepfacD);

        /// load from the backup state
        void loadSateBackupforMF();

        /**
         * remove the outliers
         */
        void removeOutliersforMF();
         ...
         * they will no longer be used in optimization
         */
        void flagPointsForRemovalforMF();
         ...
         * @param gtDepth
         */
        void makeNewTracesforMF(shared_ptr<FrameHessian> newFrame, float *gtDepth);

        /// marginalizes a frame. drops / marginalizes points & residuals.
        /// residuals will be dropped, but we still have this frame in the memory
        void marginalizeFrameforMF(shared_ptr<Frame> &frame);

        /**
         * mapping loop is running in a single thread
         */
        void mappingLoopforMF();

        /// print the residual in optimization
        void printOptResforMF(const Vec3 &res, double resL, double resM, double resPrior, double LExact, vector<AffLight> va);
    ...
    private:
        // data
        // =================== multi-fisheyey calibration geometry
        ...
        IndexThreadReduce<Vec10> threadReduce;            // multi thread reducing  多线程任务使用reduce函数对给定任务进行多线程处理
        ...
        // active residuals
        std::vector<shared_ptr<PointFrameResidual>> activeResiduals;  // 新加入的激活点的残差
        float currentMinActDist = 2;   // 激活点的阈值
