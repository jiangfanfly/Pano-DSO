...
namespace ldso {
    ...
    public:
        ...
        // for multi-fisheye
        Feature(float u, float v, float xs, float ys, float zs, shared_ptr<Frame> host, int n) 
            : host(host), uv(Vec2f(u, v)), xyzs(Vec3f(xs, ys, zs)), camnum(n) {}
